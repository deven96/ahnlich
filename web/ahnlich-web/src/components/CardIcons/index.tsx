...
/*
 * A small library of modern line icons (lucide-style) for doc-card tiles.
 * Reference an icon by name via <CardIcon name="database" />. Unknown names
 * fall back to rendering the raw string (so an emoji still works).
 */
...
export default function CardIcon({name}: {name?: string}): ReactNode {
  ...
  // fallback: render the raw string (e.g. an emoji)
  return <span style={{fontSize: '1.35rem', lineHeight: 1}}>{name}</span>;
}
