// Modern line-icon markup for sidebar category items. Used from sidebars.ts via
// `customProps: sbIcon('database')`. The DocSidebarItem/Category swizzle renders
// `iconType: 'svg'` inline, and `stroke="currentColor"` makes it match the menu
// link colour (muted / active gold) automatically.

const S = (paths: string): string =>
  `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="100%" height="100%">${paths}</svg>`;
...
const ICONS: Record<string, string> = {
  ...
  bookOpen: S('<path d="M12 7v14"/><path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z"/>'),
  database: S('<ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5v14a9 3 0 0 0 18 0V5"/><path d="M3 12a9 3 0 0 0 18 0"/>'),
  layers: S('<path d="m12 2 9 5-9 5-9-5z"/><path d="m3 12 9 5 9-5"/><path d="m3 17 9 5 9-5"/>'),
  sparkles: S('<path d="M12 3v4M12 17v4M3 12h4M17 12h4"/><path d="M12 8.5 13.4 11 16 12l-2.6 1L12 15.5 10.6 13 8 12l2.6-1z"/>'),
...
export function sbIcon(name: keyof typeof ICONS | string) {
  return {icon: ICONS[name] ?? '', iconType: 'svg'};
}
