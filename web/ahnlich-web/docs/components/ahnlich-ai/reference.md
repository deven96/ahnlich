...
| GET KEY doc1 IN article_store | ```client.get_key(GetKey { store: "article_store".into(), keys: vec![doc1], schema: None }, None).await?;``` | ```await client.get_key(GetKey(store="article_store", keys=[doc1], schema=None))``` | ```client.GetKey(ctx, &query.GetKey{Store: "article_store", Keys: []*keyval.StoreInput{doc1}})``` |
