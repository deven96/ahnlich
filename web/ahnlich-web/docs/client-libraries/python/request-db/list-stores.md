...
* The server responds with a collection of registered Stores. Each store entry includes: name, entry count, size in bytes, and non-linear index configurations (HNSW parameters) if any are active.
