# Generated by the protocol buffer compiler.  DO NOT EDIT!
# sources: ai/server.proto
# plugin: python-betterproto
# This file has been @generated

from dataclasses import dataclass
from typing import List, Optional

import betterproto

from ... import client as __client__
from ... import keyval as __keyval__
from ... import similarity as __similarity__
from ...db import server as __db_server__
from ...shared import info as __shared_info__
from .. import models as _models__


@dataclass(eq=False, repr=False)
class Unit(betterproto.Message):
    pass


@dataclass(eq=False, repr=False)
class Pong(betterproto.Message):
    pass


@dataclass(eq=False, repr=False)
class ClientList(betterproto.Message):
    clients: List["__client__.ConnectedClient"] = betterproto.message_field(1)


@dataclass(eq=False, repr=False)
class StoreList(betterproto.Message):
    stores: List["AiStoreInfo"] = betterproto.message_field(1)


@dataclass(eq=False, repr=False)
class InfoServer(betterproto.Message):
    info: "__shared_info__.ServerInfo" = betterproto.message_field(1)


@dataclass(eq=False, repr=False)
class Set(betterproto.Message):
    upsert: "__shared_info__.StoreUpsert" = betterproto.message_field(1)


@dataclass(eq=False, repr=False)
class GetEntry(betterproto.Message):
    key: "__keyval__.StoreInput" = betterproto.message_field(1)
    value: "__keyval__.StoreValue" = betterproto.message_field(2)


@dataclass(eq=False, repr=False)
class Get(betterproto.Message):
    entries: List["GetEntry"] = betterproto.message_field(1)


@dataclass(eq=False, repr=False)
class GetSimNEntry(betterproto.Message):
    key: Optional["__keyval__.StoreInput"] = betterproto.message_field(1, optional=True)
    value: "__keyval__.StoreValue" = betterproto.message_field(2)
    similarity: "__similarity__.Similarity" = betterproto.message_field(3)


@dataclass(eq=False, repr=False)
class GetSimN(betterproto.Message):
    entries: List["GetSimNEntry"] = betterproto.message_field(1)


@dataclass(eq=False, repr=False)
class Del(betterproto.Message):
    deleted_count: int = betterproto.uint64_field(1)


@dataclass(eq=False, repr=False)
class CreateIndex(betterproto.Message):
    created_indexes: int = betterproto.uint64_field(1)


@dataclass(eq=False, repr=False)
class AiStoreInfo(betterproto.Message):
    name: str = betterproto.string_field(1)
    query_model: "_models__.AiModel" = betterproto.enum_field(2)
    index_model: "_models__.AiModel" = betterproto.enum_field(3)
    embedding_size: int = betterproto.uint64_field(4)
    predicate_indices: List[str] = betterproto.string_field(5)
    dimension: int = betterproto.uint32_field(6)
    db_info: Optional["__db_server__.StoreInfo"] = betterproto.message_field(
        7, optional=True
    )


@dataclass(eq=False, repr=False)
class EmbeddingWithMetadata(betterproto.Message):
    embedding: "__keyval__.StoreKey" = betterproto.message_field(1)
    metadata: Optional["__keyval__.StoreValue"] = betterproto.message_field(
        2, optional=True
    )


@dataclass(eq=False, repr=False)
class MultipleEmbedding(betterproto.Message):
    embeddings: List["EmbeddingWithMetadata"] = betterproto.message_field(1)


@dataclass(eq=False, repr=False)
class SingleInputToEmbedding(betterproto.Message):
    input: "__keyval__.StoreInput" = betterproto.message_field(1)
    multiple: "MultipleEmbedding" = betterproto.message_field(2, group="variant")
    single: "EmbeddingWithMetadata" = betterproto.message_field(3, group="variant")


@dataclass(eq=False, repr=False)
class StoreInputToEmbeddingsList(betterproto.Message):
    values: List["SingleInputToEmbedding"] = betterproto.message_field(1)
