...
impl Store {
    ...
    /// Rebuilds the ordered index when migrating snapshots that did not persist it.
    ///
    /// # Performance
    /// This scans every store entry and rebuilds the index in O(n log n) while
    /// holding its write lock. Normal mutations must maintain the index
    /// incrementally instead of calling this method.
