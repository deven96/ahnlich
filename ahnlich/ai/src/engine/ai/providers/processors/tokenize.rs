...
use crate::engine::ai::providers::ort_helper::{HFConfigReader, read_file_to_bytes};
use crate::engine::ai::providers::processors::preprocessor::TokenizerFiles;
...
impl Tokenize {
    pub fn download_artifacts(tokenizer_files: TokenizerFiles, model_repo: ApiRepo) -> Result<TokenizeArtifacts, AIProxyError> {
        let tokenizer_bytes = read_file_to_bytes(
            &model_repo.get(&tokenizer_files.tokenizer_file)
                .map_err(|e| AIProxyError::ModelConfigLoadError{
            message: format!("failed to fetch {}, {}", &tokenizer_files.tokenizer_file, e.to_string()),
        })?)?;
    ...
    pub fn initialize(tokenizer_files: TokenizerFiles, model_repo: ApiRepo) -> Result<Self, AIProxyError> {
        let artifacts = Self::download_artifacts(tokenizer_files, model_repo)?;
        let mut tokenizer =
            Tokenizer::from_bytes(artifacts.tokenizer_bytes).map_err(|_| AIProxyError::ModelTokenizerLoadError {
        ...
        Ok(Self { tokenizer: tokenizer.into(), model_max_length, truncate: true })
    }

    pub fn set_truncate(&mut self, truncate: bool) -> Result<(), AIProxyError> {
        let tokenizer;
        if truncate {
            tokenizer = self.tokenizer.with_truncation(
            ...
            tokenizer = self.tokenizer.with_truncation(None)
                .map_err(|_| AIProxyError::ModelTokenizerLoadError {
    ...
    fn process(&self, data: PreprocessorData) -> Result<PreprocessorData, AIProxyError> {
        match data {
            PreprocessorData::Text(text) => {
                let tokenized = self.tokenizer.encode_batch(text.clone(), true)
            ...
            _ => Err(AIProxyError::ModelTokenizationError {
                message: format!("Tokenize process failed. Expected Text."),
            }),
        }
    }
}
