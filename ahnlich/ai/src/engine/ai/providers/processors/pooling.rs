use ndarray::{s, Array, Array2, ArrayView, Dim, Axis, Dimension, IxDynImpl, Ix2};
    ...
    fn process(&self, data: PostprocessorData) -> Result<PostprocessorData, AIProxyError> {
        match data {
            ...
            _ => Err(AIProxyError::PoolingError {
                message: "Expected NdArray3, got NdArray2".to_string(),
            }),
        }
    }
}
            ...
            PostprocessorData::NdArray3(array) => {
                let attention_mask = match &self.attention_mask {
                    Some(mask) => {
                        ...
                        attention_mask
                            .insert_axis(Axis(2))
                            .broadcast(array.dim()).ok_or(
                            AIProxyError::PoolingError {
                                message: format!("Could not broadcast attention mask with shape {:?} to \
                         shape {:?} of the input tensor.", mask.shape(), array.shape()),
                            }
                        )?.to_owned()
                    },
                ...
                Ok(PostprocessorData::NdArray2(&masked_array_sum / &attention_mask_sum))
            }
            _ => Err(AIProxyError::PoolingError {
                message: "Expected NdArray3, got NdArray2".to_string(),
            }),
