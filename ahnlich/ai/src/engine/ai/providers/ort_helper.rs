...
pub fn read_file_to_bytes(file: &PathBuf) -> Result<Vec<u8>, AIProxyError> {
    let mut file = File::open(file).map_err(|_| AIProxyError::ModelTokenizerLoadError)?;
    let file_size = file.metadata().map_err(|_| AIProxyError::ModelTokenizerLoadError)?.len() as usize;
    let mut buffer = Vec::with_capacity(file_size);
    file.read_to_end(&mut buffer).map_err(|_| AIProxyError::ModelTokenizerLoadError)?;
    Ok(buffer)
}