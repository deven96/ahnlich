...
impl HFConfigReader {
    ...
    pub fn read(&mut self, config_name: &str) -> Result<serde_json::Value, AIProxyError> {
        ...
        let file = self.model_repo.get(config_name).map_err(|_| AIProxyError::ModelConfigLoadError{
            message: format!("failed to parse {}", config_name),
        })?;
        let contents = read_file_to_bytes(&file).unwrap();
        let value: serde_json::Value = serde_json::from_slice(&contents).map_err(
            |_| AIProxyError::ModelConfigLoadError{
            message: format!("failed to parse {}", config_name),
        })?;
